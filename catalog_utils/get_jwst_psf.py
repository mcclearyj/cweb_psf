...
def make_starcat(image_file, config, star_params=None, thresh=0.55, cat_file=None, train_size=0.8):
    '''
    ...
    return starcat_name
    '''
    indices = np.random.permutation(len(star_cat))
    train_indices = indices[:int(len(indices) * train_size)]
    valid_indices = indices[int(len(indices) * train_size):]
    ...
    valid_starcat_name = os.path.join(
                outdir, img_basename.replace('.fits','_valid_starcat.fits')
                )

    # write the training catalog to a file
    im_cat_fits['LDAC_OBJECTS'].data = train_star_cat
    im_cat_fits.writeto(train_starcat_name, overwrite=True)

    # write the validation catalog to a file
    im_cat_fits['LDAC_OBJECTS'].data = valid_star_cat
    im_cat_fits.writeto(valid_starcat_name, overwrite=True)

    # Make size-mag plot
    size_mag_plots(im_cat, train_star_cat, plot_name.replace('.pdf', '_train.pdf'), filter_name)
    size_mag_plots(im_cat, valid_star_cat, plot_name.replace('.pdf', '_valid.pdf'), filter_name)

    return train_starcat_name, valid_starcat_name
...
def split_extension2_into_training_validation(input_file, training_fraction=0.9):
    # Open the FITS file
    with fits.open(input_file) as hdul:
        # Copy extension 1 exactly
        extension1 = hdul[1].copy()

        # Get the data from extension 2
        extension2_data = hdul[2].data
        extension2_header = hdul[2].header

        # Determine the number of rows for the training and validation catalogs
        total_rows = len(extension2_data)
    ...
    return training_hdul, validation_hdul
...
def main(args):
    ...
    for j, i2d in enumerate(i2d_images):
        ...
        starcat_file, valid_file = make_starcat(image_file=image_file, config=config, star_params=star_params, cat_file=cat_file)
        print(starcat_file)
