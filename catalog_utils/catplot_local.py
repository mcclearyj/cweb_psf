...
class resid_plot(plot):
    ...
    def preprocessing(self, hsm_fit=False):
      ...
      #  for i in range(len(stars)):
       #     stars[i] = stars[i]/np.nansum(stars[i])
        ...
        self.avg_star = np.nanmedian(self.cropped_stars, axis=0)
        ...
        self.cropped_psfs = psfs
        self.avg_psf = np.nanmedian(self.cropped_psfs, axis=0)
        if hsm_fit:
            for starstamp in stars:
                starstamp[starstamp<= -1000] = np.nan
                gs_object = galsim.Image(starstamp, wcs=galsim.PixelScale(0.03), xmin=0, ymin=0)
                try:
            ...
            for psfstamp in self.cropped_psfs:
                gs_object = galsim.Image(psfstamp, wcs=galsim.PixelScale(0.03), xmin=0, ymin=0)
                try:
                    HSM_fit=gs_object.FindAdaptiveMom()
                    self.psf_hsm.append(1)
                except:
                    try:
                        gs_object = galsim.Image(psfstamp+abs(np.min(psfstamp)), wcs=galsim.PixelScale(0.03), xmin=0, ymin=0)
                        HSM_fit=gs_object.FindAdaptiveMom()
                        self.psf_hsm.append(1)
                    except:
    ...
    def save_figure(self, outname, vminscale=1, vmaxscale=1, linthresh=0.05):
        #vmin = -0.1 #np.nanmin(self.avg_psf)
        vmin = vminscale*np.minimum(np.nanmin(self.avg_star),
                                    np.nanmin(self.avg_psf))
        vmax = vmaxscale*np.maximum(np.nanmax(self.avg_star),
                                    np.nanmax(self.avg_psf))
        ...
        norm = colors.SymLogNorm(vmin=vmin, vmax=vmax, linthresh=linthresh)
        star_norm = colors.SymLogNorm(vmin=vmin, vmax=vmax, linthresh=1e-4)
        psf_norm = colors.SymLogNorm(vmin=vmin, vmax=vmax, linthresh=1e-2)
    ...
    def set_residuals(self):
        ...
        for i in range(len(stars)):
            ...
            for j in range(stars[0].shape[0]):
                for k in range(stars[0].shape[1]):
                    relative_error[j,k] = \
                        (stars[i][j,k] - psfs[i][j,k])/(stars[i][j,k] + 1.0e-6)
    ...
    def set_residuals(self):
        ...
        for i in range(len(stars)):
            ...
            for j in range(stars[0].shape[0]):
                for k in range(stars[0].shape[1]):
                    absolute_error[j,k] = \
                        np.abs(stars[i][j,k] - psfs[i][j,k])/np.abs(stars[i][j,k] + 1.0e-6)
...
class chi_2_error_plot(resid_plot):
    ...
    def set_residuals(self, polydim=2, polydeg=1):
        ...
        self.avg_residual = np.ma.mean(np.ma.masked_invalid(resids),
                            axis=0).data
        self.set_residuals_sum()
